 ...
 */
@WebSocket(
        maxTextMessageSize = 256 * 1024 * 1024)
public class ServiceSocket {
    ...
    public void onMessage(String msg) {
        synchronized (parent) {
            ...
            logMessage.append(" - Received message #")
                    .append(messageCounter)
                    .append(length);
            addResponseMessage("[Message " + (messageCounter++) + "]\n" + msg + "\n\n");

            if (responseExpression == null || responseExpression.matcher(msg)
                    .find()) {
                logMessage.append("; matched response pattern")
                        .append("\n");
                closeLatch.countDown();
            } else if (!disconnectPattern.isEmpty() && disconnectExpression.matcher(msg)
                    .find()) {
    ...
    public void onWebSocketBinary(byte[] payload, int offset, int len) {
        ...
        synchronized (parent) {
            ...
            String length = " (" + msg.length() + " bytes)";
            logMessage.append(" - Received binary message #")
                    .append(messageCounter)
                    .append(length);
            addResponseMessage("[Message " + (messageCounter++) + "]\n" + msg + "\n\n");
    ...
    public void onFrame(Frame frame) {
        synchronized (parent) {
            log.debug("Received frame: " + frame.getPayload() + " " + frame.getType()
                    .name());
            String length = " (" + frame.getPayloadLength() + " bytes)";
            logMessage.append(" - Received frame #")
                    .append(messageCounter)
                    .append(length);
            String frameTxt = new String(frame.getPayload()
                    .array());
            addResponseMessage("[Frame " + (messageCounter++) + "]\n" + frameTxt + "\n\n");

            if (responseExpression == null || responseExpression.matcher(frameTxt)
                    .find()) {
                logMessage.append("; matched response pattern")
                        .append("\n");
                closeLatch.countDown();
            } else if (!disconnectPattern.isEmpty() && disconnectExpression.matcher(frameTxt)
                    .find()) {
                logMessage.append("; matched connection close pattern")
                        .append("\n");
                closeLatch.countDown();
    ...
    public void onOpen(Session session) {
        logMessage.append(" - WebSocket conection has been opened")
                .append("\n");
    ...
    public boolean awaitClose(int duration, TimeUnit unit) throws InterruptedException {
        logMessage.append(" - Waiting for messages for ")
                .append(duration)
                .append(" ")
    ...
    public boolean awaitOpen(int duration, TimeUnit unit) throws InterruptedException {
        logMessage.append(" - Waiting for the server connection for ")
                .append(duration)
                .append(" ")
                .append(unit.toString())
                .append("\n");
        ...
        if (connected) {
            logMessage.append(" - Connection established")
                    .append("\n");
        } else {
            logMessage.append(" - Cannot connect to the remote server")
                    .append("\n");
        }
    ...
    public void sendMessage(String message) throws IOException {
        if (!message.startsWith("hex:")) {
            ...
            try {
                byte[] decodeHex = Hex.decodeHex(message.replace("hex:", "")
                        .toCharArray());
                session.getRemote()
                        .sendBytes(ByteBuffer.wrap(decodeHex));
            } catch (DecoderException e) {
                logMessage.append("Can not decode hex value - " + e.getMessage())
                        .append("\n");
            }
        }
    }
    ...
    public String getLogMessage() {
        logMessage.append("\n\n[Variables]\n");
        logMessage.append(" - Message count: ")
                .append(messageCounter - 1)
                .append("\n");
    ...
    protected void initializePatterns() {
        try {
            logMessage.append(" - Using response message pattern \"")
                    .append(responsePattern)
                    .append("\"\n");
            responseExpression =
                    (responsePattern != null || !responsePattern.isEmpty()) ? Pattern.compile(responsePattern) : null;
        } catch (Exception ex) {
        ...
        try {
            logMessage.append(" - Using disconnect pattern \"")
                    .append(disconnectPattern)
                    .append("\"\n");
            disconnectExpression = (disconnectPattern != null || !disconnectPattern.isEmpty())
                    ? Pattern.compile(disconnectPattern) : null;
        } catch (Exception ex) {
    ...
    private void addResponseMessage(String message) {
        ...
        try {
            ...
            logMessage.append(" - Message backlog value not set; using default ")
                    .append(WebSocketSampler.MESSAGE_BACKLOG_COUNT)
                    .append("\n");
